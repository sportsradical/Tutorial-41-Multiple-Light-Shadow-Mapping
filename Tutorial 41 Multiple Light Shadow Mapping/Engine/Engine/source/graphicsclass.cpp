...
bool GraphicsClass::Initialize(int screenWidth, int screenHeight, HWND hwnd)
{
	...
	result = m_CubeModel->Initialize(m_D3D->GetDevice(), m_D3D->GetDeviceContext(), "../Engine/data/cube.txt", "../Engine/data/wall01.tga");
	if(!result)
	{
	...
	result = m_SphereModel->Initialize(m_D3D->GetDevice(), m_D3D->GetDeviceContext(), "../Engine/data/sphere.txt", "../Engine/data/ice.tga");
	if(!result)
	{
	...
	result = m_GroundModel->Initialize(m_D3D->GetDevice(), m_D3D->GetDeviceContext(), "../Engine/data/plane01.txt", "../Engine/data/metal001.tga");
	if(!result)
	{
...
bool GraphicsClass::Frame(float posX, float posY, float posZ, float rotX, float rotY, float rotZ)
{
	...
	m_Light->SetPosition(5.0f, 8.0f, -15.0f);

	// Set the position of the second light.
	m_Light2->SetPosition(-5.0f, 8.0f, -15.0f);
